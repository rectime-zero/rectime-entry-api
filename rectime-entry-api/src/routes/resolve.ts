...
resolveRoute.post('/v1/resolve', async (c) => {
  ...
  if (!bearerToken) {
    throw new AppError(401, 'UNAUTHORIZED', '隱崎ｨｼ繝医・繧ｯ繝ｳ縺御ｸ肴ｭ｣縺ｧ縺・')
  }
...
async function parseResolveBody(request: Request): Promise<ResolveRequestBody> {
  ...
  try {
    ...
    throw new AppError(400, 'BAD_REQUEST', '繝ｪ繧ｯ繧ｨ繧ｹ繝医・繝・ぅ縺御ｸ肴ｭ｣縺ｧ縺・')
  }
  ...
  if (typeof appVersion !== 'string' || appVersion.length === 0) {
    throw new AppError(400, 'BAD_REQUEST', 'appVersion 縺御ｸ肴ｭ｣縺ｧ縺・')
  }

  if (platform !== 'ios' && platform !== 'android' && platform !== 'web') {
    throw new AppError(400, 'BAD_REQUEST', 'platform 縺御ｸ肴ｭ｣縺ｧ縺・')
  }
...
async function parseResolveByEmailBody(request: Request): Promise<ResolveByEmailRequestBody> {
  ...
  if (typeof email !== 'string' || email.trim().length === 0) {
    throw new AppError(400, 'BAD_REQUEST', 'email 縺御ｸ肴ｭ｣縺ｧ縺・')
  }
